...
/// The customer app router: signed-in shell plus public auth screens.
final appRouterProvider = Provider<GoRouter>((ref) {
  final router = GoRouter(
    ...
    errorBuilder: (_, state) => NotFoundPage(path: state.uri.path),
  );
  ref.onDispose(router.dispose);
  return router;
});

/// Unknown-route fallback (PF-DOC-17 §3.3).
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: PfEmptyState(
        ...
        subtitle: 'Alamat "$path" tidak dikenal.',
      ),
    );
  }
}
