...
PareBisnis merchant app — thin composition root (MO-R06, PF-DOC-06).

Sprint 1 scope: role-guarded auth (`business` only), restaurant
onboarding/verification status and menu management behind a three-tab shell
(Restoran, Menu, Akun). No business logic lives here; screens come from
...
| `lib/src/router/app_router.dart` | GoRouter + auth/role guards (PF-DOC-17). |
| `lib/src/shell/` | Three-tab shell: Restoran (status/onboarding), Menu, Akun. |
| `lib/src/presentation/access_denied_page.dart` | Role-mismatch screen. |
